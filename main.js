...
Game.Launch=function()
{
	...
	'<div class="listing">&bull; 增加了挑战模式，可以在转生时选择 (现在只有1: "重生")</div>'+
	...
	Game.Init=function()
	{
		...
		//if you're making a mod, don't forget to add a Game.Win('第三方') somewhere in there!
		...
		Game.LoadSave=function(data)
		{
			...
			if (str!='')
			{
				...
				if (str!='')
				{
					...
					if (version>=1)
					{
						...
						if (Game.mobile || Game.Has('完美的空转') || Game.Has('双重超越之门'))
						{
							...
							else
							{
								...
								if (Game.Has('领土')) percent+=10;
								if (Game.Has('小天使')) percent+=10;
					...
					{
						Game.Notify('导入错误','对不起，您不能导入旧版本的存档。','',6,1);
						return false;
					}
		...
		Game.harvestLumps=function(amount,silent)
		{
			...
			else if (Game.lumpCurrentType==2)
			{
				...
				if (Game.prefs.popups) Game.Popup('糖的祝福激活!');
			...
			if (!silent)
			{
				...
				if (yield>0) Game.Popup('<small>+'+Beautify(yield)+' 糖块'+(yield==1?'':'')+'</small>',(rect.left+rect.right)/2,(rect.top+rect.bottom)/2-48);
		...
		Game.CalculateGains=function()
		{
			...
			if (Game.Has('"蛋"')) {Game.cookiesPs+=9;Game.cookiesPsByType['"egg"']=9;}//"egg"
		...
		Game.UpdateMenu=function()
		{
			...
			else if (Game.onMenu=='stats')
			{
				...
				var wrathStr='';
				if (Game.elderWrath==1) wrathStr='唤醒';
				else if (Game.elderWrath==2) wrathStr='不高兴';
				else if (Game.elderWrath==3) wrathStr='愤怒';
				else if (Game.elderWrath==0 && Game.pledges>0) wrathStr='平息';
				...
				(Game.resets?('<div class="listing"><b>遗产开始 :</b> '+(fullDate==''?'刚才':(fullDate+' 之前'))+', 重生了 '+Beautify(Game.resets)+' 次'+(Game.resets==1?'':'s')+'</div>'):'')+
				'<div class="listing"><b>游戏运行时间 :</b> '+(startDate==''?'刚才':(startDate+' 之前'))+'</div>'+
				...
				'<div class="listing"><b>每秒生产饼干数 :</b> '+Beautify(Game.cookiesPs,1)+' <small>'+
					...
					(Game.cpsSucked>0?' <span class="warning">(枯萎的 : '+Beautify(Math.round(Game.cpsSucked*100),1)+'%)</span>':'')+
					'</small></div>'+
				...
				(wrathStr!=''?'<div class="listing"><b>老奶奶的地位 :</b> '+wrathStr+'</div>':'')+
				...
				'<div class="listing"><div class="icon" style="float:left;background-position:'+(-19*48)+'px '+(-7*48)+'px;"></div>'+
					'<div style="margin-top:8px;"><span class="title" style="font-size:22px;">声望等级 : '+Beautify(Game.prestige)+'</span> 在 '+Beautify(heavenlyMult*100,1)+'% 默认 <b>(+'+Beautify(parseFloat(Game.prestige)*Game.heavenlyPower*heavenlyMult,1)+'% 每秒饼干的产量)</b><br><br>天堂芯片 : <b>'+Beautify(Game.heavenlyChips)+'</b></div>'+
				'</div>'+
				(prestigeUpgrades!=''?(
		...
		new Game.Upgrade('巧克力蛋','包含 <b>很多的饼干</b>.<br>成本是你拥有多少个蛋。<q>这是一种难以捉摸的可可鸟。里面有个惊喜!</q>',eggPrice2,[18,12],function()
		{
			...
			else Game.Notify('巧克力蛋','鸡蛋突然爆发 <b>'+Beautify(cookies)+'</b> 饼干！',Game.Upgrades['巧克力蛋'].icon);
			Game.Earn(cookies);
		});
		...
		Game.AssignPermanentSlot=function(slot)
		{
			...
			Game.Prompt('<h3>选择升级，使其成为永久的</h3>'+
						...
						'<div class="block" style="float:right;width:152px;clear:right;height:234px;">以下是你上次购买的所有升级。<div class="line"></div>选择一个来永久地获得它的效果！<div class="line"></div>当你转生时，你可以重新分配这个位置。</div>'
						,[['确认','Game.permanentUpgrades['+slot+']=Game.SelectingPermanentUpgrade;Game.BuildAscendTree();Game.ClosePrompt();'],'取消'],0,'widePrompt');
		}
		...
		var desc=function(percent,total){return '你获得了另外 <b>+'+percent+'%</b> 当前饼干每秒产量, 总共 <b>'+total+'%</b>.';}
		...
		new Game.Upgrade('领土',desc(10,45)+'<q>在糕点天堂的第二个领域，自治领有一个管理职位，负责会计和管理的日程安排。</q>',Math.pow(angelPriceFactor,4),[3,11]);Game.last.pool='prestige';Game.last.parents=['美德'];
		new Game.Upgrade('小天使',desc(10,55)+'<q>在糕点天堂的第一层，四面派的小天使是天堂的护卫和保镖。</q>',Math.pow(angelPriceFactor,5),[4,11]);Game.last.pool='prestige';Game.last.parents=['领土'];
		...
		order=20000;
		new Game.Upgrade('小猫会计','你获得了 <b>更多的饼干每秒产量</b> 你的牛奶越多。<q>生意很好，先生</q>',900000000000000000000000,Game.GetIcon('Kitten',6));Game.last.kitten=1;
		new Game.Upgrade('小猫专家','你获得了 <b>更多的饼干每秒产量</b> 你的牛奶越多。<q>乐观地调整你的工作流程，先生</q>',900000000000000000000000000,Game.GetIcon('Kitten',7));Game.last.kitten=1;
		new Game.Upgrade('小猫能手','你获得了 <b>更多的饼干每秒产量</b> 你的牛奶越多。<q>先生，10年时间在饼干生意中度过了</q>',900000000000000000000000000000,Game.GetIcon('Kitten',8));Game.last.kitten=1;
		...
		new Game.SynergyUpgrade('上帝粒子','<q>Turns out 上帝 is much tinier than we thought, I guess.</q>','Temple','Antimatter condenser','synergy2');
		...
		new Game.Upgrade('协同效应1','解锁 a new tier of upgrades that affect <b>2 buildings at the same time</b>.<br>Synergies appear once you have <b>15</b> of both buildings.<q>The many beats the few.</q>',2525,[10,20]);Game.last.pool='prestige';Game.last.parents=['撒旦','领土'];
		new Game.Upgrade('协同效应2','解锁 a new tier of upgrades that affect <b>2 buildings at the same time</b>.<br>Synergies appear once you have <b>75</b> of both buildings.<q>The several beats the many.</q>',252525,[10,20]);Game.last.pool='prestige';Game.last.parents=['恶魔','六翼天使','协同效应1'];
		...
		Game.last.choicesFunction=function()
		{
			var choices=[];
			choices[0]={name:'自动',icon:[0,7]};
			choices[1]={name:'蓝色',icon:[21,21]};
			choices[2]={name:'红色',icon:[22,21]};
			choices[3]={name:'白色',icon:[23,21]};
			choices[4]={name:'黑色',icon:[24,21]};
			choices[5]={name:'黄金',icon:[25,21]};
			choices[6]={name:'老奶奶',icon:[26,21]};
			choices[7]={name:'生气的老奶奶',icon:[27,21]};
			choices[8]={name:'愤怒的老奶奶',icon:[28,21]};
			choices[9]={name:'钱',icon:[29,21]};
		...
		*/
		...
		new Game.buffType('sugar blessing',function(time,pow)
		{
			return {
				name:'糖的祝福',
				desc:'你发现黄金饼干的几率增加 10% 接下来 '+Game.sayTime(time*Game.fps,-1)+'.',
				icon:[29,16],
				time:time*Game.fps,
				//add:true
			};
		...
		Game.dragonLevels=[
			...
			{name:'Krumblor, cookie dragon',action:'训练地球毁灭者<br><small>光环:建筑以85%而不是50%的价格出售</small>',pic:5,
			...
			{name:'Krumblor, cookie dragon',action:'训练祖先的蜕变<br><small>光环:金色的饼干多给10%的饼干</small>',pic:5,
				...
				costStr:function(){return '100 炼金实验室';}},
			...
			{name:'Krumblor, cookie dragon',action:'训练Radiant Appetite<br><small>光环:所有的饼干产量乘以2</small>',pic:5,
		...
		{	
			...
			Game.Prompt('<h3>设置你的龙的 '+(slot==1?'次要的 ':'')+'光环</h3>'+
						...
						'<div class="line"></div>'+
						'<div style="text-align:center;margin-bottom:8px;">'+(highestBuilding==0?'转换你的光环是<b>免费</b>，因为你没有建筑。':'改变你的光环的代价是 <b>1 '+highestBuilding.name+'</b>.<br>这会影响到你的饼干每秒产量 !')+'</div>'
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.researchT==0 && Game.nextResearch)
			{
				if (!Game.Has(Game.UpgradesById[Game.nextResearch].name))
				{
					...
					if (Game.prefs.popups) Game.Popup('研究 : '+Game.UpgradesById[Game.nextResearch].name);
					else Game.Notify('研究完成','你已经发现 : <b>'+Game.UpgradesById[Game.nextResearch].name+'</b>.',Game.UpgradesById[Game.nextResearch].icon);
				}
				Game.nextResearch=0;
				Game.researchT=-1;
				Game.recalculateGains=1;
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.Has('永恒的心饼干')) Game.Win('可爱的饼干');
				if (Game.season=='easter')
				{
					var eggs=0;
				...
				if (minAmount>=300) {Game.Win('三百周年纪念');Game.Unlock('薰衣草巧克力奶油饼干');}
